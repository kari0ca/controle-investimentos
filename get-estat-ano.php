<?php
	...
	//Query datas
	$query_data = 'select ano_ref from investdb.inv_agg_ano where idcarteira in (';
	...
	//Query valores
	$query_val = 'select ano_ref, idcarteira, rend_ano_perc from investdb.inv_agg_ano where idcarteira in (';
	...
	$query_val .= ')';
	$query_val .= ' and ano_ref >= '.$datas[$cont_data-1].' and ano_ref <= '.$datas[0].' group by ano_ref, idcarteira order by ano_ref desc;';
	...
	while($fim == 0){
		...
		$val_data = $row[ano_ref];
		$val_id_cart = $row[idcarteira];
		$val_invest = $row[rend_ano_perc];
